...
详细参数说明请参考：[生产环境测试与私钥导入指南](./Production_Testing_and_Wallet_Import.md)
...
### 集群自动识别
SDK 对 `cluster` 参数具备容错性，你可以输入 `mainnet`、`Devnet` 等常用缩写，内部会自动映射并校验。

### 多路私钥导入
在你的测试脚本中，可以使用以下方式快速切换身份：
1. **环境变量**: `SOLANA_PRIVATE_KEY`
2. **命令行参数**: `--keypair /path/to/key.json`
