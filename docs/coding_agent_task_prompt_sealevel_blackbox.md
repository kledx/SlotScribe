...
### 3.2 Blackbox SDK（`src/blackbox/*`）

#### `src/blackbox/types.ts`
定义类型（足够 MVP）：
...
#### `src/blackbox/solana.ts`
提供 Solana 相关 helper：

- `getRpcUrl(cluster, overrideUrl?)`
- `buildMemoIx(memo: string): TransactionInstruction`
- `extractBlackboxMemo(memoData: string): { payloadHash?: string; raw: string }`
...
#### `app/layout.tsx`
最简 layout，显示标题“Sealevel Blackbox”
